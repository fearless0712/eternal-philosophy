    ...
    <div ref={root} className={styles.experience} data-ready={ready}>
      ...
      <main>
        ...
        <section id="works" className={styles.works}>
          ...
          <div className={styles.projectList}>{featuredProjects.map((project, index) => <article className={styles.project} key={project.slug}><span className={styles.projectNumber}>0{index + 1}</span><h3><Link href={`/works/${project.slug}`}>{project.title}</Link></h3><p>{project.category}</p><time>{project.year}</time><span className={styles.projectSummary}>{project.content[locale].shortDescription}</span><div className={styles.projectLinks}><Link href={`/works/${project.slug}`}>VIEW ↗</Link>{project.liveUrl && <a href={project.liveUrl} target="_blank" rel="noopener noreferrer">LIVE ↗</a>}{project.githubUrl && <a href={project.githubUrl} target="_blank" rel="noopener noreferrer">GITHUB ↗</a>}</div></article>)}</div>
        </section>
        ...
        <section id="contact" className={styles.contact}><p>06 / CONTACT</p><h2>LET&apos;S BUILD<br />WHAT DOESN&apos;T<br />EXIST YET.</h2><nav className={styles.contactLinks} aria-label="External contact links">{siteConfig.email.length > 0 && <a href={`mailto:${siteConfig.email}`}>EMAIL <span>↗</span></a>}<a href={siteConfig.githubUrl} target="_blank" rel="noopener noreferrer">GITHUB <span>↗</span></a>{siteConfig.linkedinUrl.length > 0 && <a href={siteConfig.linkedinUrl} target="_blank" rel="noopener noreferrer">LINKEDIN <span>↗</span></a>}</nav><footer><span>ETERNAL PHILOSOPHY</span><span>{siteConfig.location.toUpperCase()} — 2026</span></footer></section>
      </main>
